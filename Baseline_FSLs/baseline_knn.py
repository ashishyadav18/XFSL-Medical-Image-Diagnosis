...
# =========================
# BASELINE LOGIC: k-NN (k=3)
# =========================
def evaluate_knn(X_support, y_support, X_query, y_query):
    # k-Nearest Neighbors using Cosine Distance
    # We use k=3 to allow for a majority vote without being too broad
...
def main():
    ...
    print(f"\nRunning k-NN (k=3) Baseline (K={K_SHOT}, Runs={RUNS})...")
    print("-" * 65)
    ...
    for i in range(RUNS):
        ...
        # --- EVALUATE BASELINE ---
        acc, f1, mcc = evaluate_knn(X_support, y_support, X_query, y_query)
    ...
    print("-" * 65)
    print(f"🏁 FINAL k-NN RESULTS (K={K_SHOT})")
    print("=" * 65)
    ...
    # Save detailed logs
    csv_path = "logs/baseline_knn.csv"
