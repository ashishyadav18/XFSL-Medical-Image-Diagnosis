...
# =========================
# BASELINE LOGIC: RANDOM FOREST
# =========================
def evaluate_random_forest(X_support, y_support, X_query, y_query):
    # Random Forest Classifier (100 Trees)
    # random_state ensures reproducibility across runs
    rf = RandomForestClassifier(n_estimators=100, random_state=SEED)
    rf.fit(X_support, y_support)
    
    y_pred = rf.predict(X_query)
...
def main():
    ...
    print(f"\nRunning Random Forest Baseline (K={K_SHOT}, Runs={RUNS})...")
    print("-" * 65)
    ...
    for i in range(RUNS):
        ...
        # --- EVALUATE BASELINE ---
        acc, f1, mcc = evaluate_random_forest(X_support, y_support, X_query, y_query)
    ...
    print("-" * 65)
    print(f"🏁 FINAL RANDOM FOREST RESULTS (K={K_SHOT})")
    print("=" * 65)
    ...
    # Save detailed logs
    csv_path = "logs/baseline_random_forest.csv"
