...
# =========================
# BASELINE LOGIC: LINEAR SVM
# =========================
def evaluate_svm(X_support, y_support, X_query, y_query):
    # Linear Support Vector Machine
    # random_state ensures reproducibility in any ties/optimization steps
    svm = SVC(kernel='linear', random_state=SEED)
    svm.fit(X_support, y_support)
    
    y_pred = svm.predict(X_query)
...
def main():
    ...
    print(f"\nRunning Linear SVM Baseline (K={K_SHOT}, Runs={RUNS})...")
    print("-" * 65)
    ...
    for i in range(RUNS):
        ...
        # --- EVALUATE BASELINE ---
        acc, f1, mcc = evaluate_svm(X_support, y_support, X_query, y_query)
    ...
    print("-" * 65)
    print(f"🏁 FINAL LINEAR SVM RESULTS (K={K_SHOT})")
    print("=" * 65)
    ...
    # Save detailed logs
    csv_path = "logs/baseline_svm.csv"
