...
# =========================
# BASELINE LOGIC: VANILLA PROTONET
# =========================
def evaluate_vanilla_protonet(X_support, y_support, X_query, y_query):
    # Vanilla ProtoNet uses the simple mean of ALL support samples (no outlier dropping)
    classes = np.unique(y_support)
    vanilla_protos = {cls: np.mean(X_support[y_support == cls], axis=0) for cls in classes}
    
    y_pred = []
    for x_q in X_query:
        # Vanilla ProtoNet classifies using Euclidean Distance (minimum distance wins)
        best_c = min(vanilla_protos.keys(), key=lambda c: euclidean(x_q, vanilla_protos[c]))
...
def main():
    ...
    print(f"\nRunning Vanilla ProtoNet Baseline (K={K_SHOT}, Runs={RUNS})...")
    print("-" * 65)
    ...
    for i in range(RUNS):
        ...
        # --- EVALUATE BASELINE ---
        acc, f1, mcc = evaluate_vanilla_protonet(X_support, y_support, X_query, y_query)
    ...
    print("-" * 65)
    print(f"🏁 FINAL VANILLA PROTONET RESULTS (K={K_SHOT})")
    print("=" * 65)
    ...
    # Save detailed logs
    csv_path = "logs/baseline_vanilla_protonet.csv"
